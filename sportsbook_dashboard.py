...
try:
    ...
    log_error(f"Failed to import from shared.py: {str(e)}")
    traceback.print_exc()
    st.stop()
...
try:
    ...
    log_error(f"Failed to import from promo_store.py: {str(e)}")
    traceback.print_exc()
    st.stop()
...
try:
    ...
    log_error(f"Failed to set page config: {str(e)}")
    traceback.print_exc()
...
try:
    ...
    log_error(f"Failed to set styling: {str(e)}")
    traceback.print_exc()
...
try:
    ...
    log_error(f"Failed to initialize session state: {str(e)}")
    traceback.print_exc()
    st.stop()
    ...
    try:
        ...
        if selection_type == "🏟️ Συγκεκριμένος Αγώνας":
            try:
                ...
                st.warning(f"Σφάλμα κατά τη φόρτωση αγώνων: {str(e)}")
            ...
            try:
                ...
                st.warning(f"Σφάλμα κατά τη φόρτωση πρωταθλημάτων: {str(e)}")
        ...
        st.error(f"Σφάλμα στη φόρμα promo: {str(e)}")
        traceback.print_exc()
        return None
...
def view_promos_dialog():
    try:
        ...
        st.error(f"Σφάλμα προβολής promos: {str(e)}")
        traceback.print_exc()
...
try:
    ...
    if st.sidebar.button("🔄 Ανανέωση Δεδομένων", use_container_width=True):
        try:
            ...
            st.error(f"Σφάλμα ανανέωσης δεδομένων: {str(e)}")
    ...
    log_error(f"Error in sidebar: {str(e)}")
    traceback.print_exc()
...
try:
    ...
    log_error(f"Error in Odds API setup: {str(e)}")
    traceback.print_exc()
...
try:
    ...
    log_error(f"Error in league filtering: {str(e)}")
    traceback.print_exc()
    leagues_to_fetch = []
...
try:
    ...
    log_error(f"Error in main display: {str(e)}")
    traceback.print_exc()
...
try:
    ...
    log_error(f"Error in data fetch/display: {str(e)}")
    traceback.print_exc()
